defmodule SpectreLens do
  ...
  def act(tab, {:navigate, ref}, opts) do
    safe_action(fn ->
      with {:ok, url} <- navigation_url(tab, ref, opts) do
  ...
  def act(tab, {:click, opts}, call_opts) when is_list(opts) do
    safe_action(fn ->
      with {:ok, ref} <- clickable_ref(tab, opts, call_opts) do
  ...
  @spec navigation_url(Tab.t(), term(), keyword()) :: {:ok, binary()} | {:error, term()}
  defp navigation_url(_tab, url, _opts) when is_binary(url), do: {:ok, url}

  defp navigation_url(_tab, %SpectreLens.ActionRef{kind: :link, href: url}, _opts)
       when is_binary(url),
       do: {:ok, url}

  defp navigation_url(_tab, %{"href" => href}, _opts) when is_binary(href), do: {:ok, href}
  defp navigation_url(_tab, %{href: href}, _opts) when is_binary(href), do: {:ok, href}

  defp navigation_url(%Tab{} = tab, query, opts) when is_list(query) do
    with {:ok, link} <- matching_link(tab, query, opts),
         href when is_binary(href) <- MapHelpers.get(link, :href) do
      {:ok, href}
    else
      _ -> {:error, SpectreLens.ElementNotFoundError.new(query)}
    end
  end

  defp navigation_url(_tab, other, _opts), do: {:error, {:missing_navigation_url, other}}

  @spec clickable_ref(Tab.t(), keyword(), keyword()) :: {:ok, term()} | {:error, term()}
  defp clickable_ref(%Tab{} = tab, opts, call_opts) do
    cond do
      ...
      true ->
        {:error, SpectreLens.ElementNotFoundError.new(opts)}
    end
  end
